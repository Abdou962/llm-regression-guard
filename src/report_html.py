...
"""
    ...
    html = html.format(
        regression_count=len(regressions_list), improvement_count=len(improvements_list), len_curr=len(curr_data)
    )
    ...
    html += (
        """</div><div class=\"section\"><h2 class=\"section-title\">Performance Trend</h2><div class=\"trend-chart\">"""
    )
    if len(trend_data) > 1:
